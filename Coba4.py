...
for name in names:
    ...
    ret, thresh = cv.threshold(img_gray, 128, 255, cv.THRESH_BINARY)
...
for i in np.arange(0, np.prod(xyz_dims), 1, dtype=np.int):
    ...
    poly_points = rules.to_points(point_group, point_type, variant=1)
    position = []
    ...
    for poly_face in rules.to_faces(position, point_type, variant=2):
...
print(polyline.volume)

plot = pv.Plotter()
plot.add_mesh(polyline, show_edges=False)
...
# -------- Attempt Start 2
# polyline1 = pv.PolyData(points_p, faces=np.array(faces_p).flatten())
# print(polyline1.is_all_triangles, polyline1.triangulate(), polyline1.volume)
# polyline1.save('../objs/oas2.stl')
# print(polyline1.is_manifold, polyline1.is_all_triangles)
# -------- Attempt End 2
